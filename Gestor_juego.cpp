...
int Gestor_juego::juego_ia(int colum, bool jugadores, char tablero[]){
   ...
   if(jugadores == false){
      for(i = 0 ;i < numeros.size() ; i = i + 4){
         if(tablero[numeros[i]] == 'O' or tablero[numeros[i]] == 'X'
            && tablero[numeros[i + 1]]=='O' or tablero[numeros[i + 1]] == 'X'
            && tablero[numeros[i+2]] =='O' or tablero[numeros[i + 2]]  == 'X'){  // Trata de ganar y e
